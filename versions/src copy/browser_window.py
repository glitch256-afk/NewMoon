...
from PyQt6.QtGui import QAction, QKeySequence, QPalette, QIcon, QShortcut
from PyQt6.QtWidgets import (
    QMainWindow, QVBoxLayout, QHBoxLayout, QWidget, 
    QLineEdit, QPushButton, QStatusBar, QProgressBar, 
    QTabWidget, QToolButton, QFileDialog
)
...
from src.download_manager import DownloadManagerDialog


def get_asset_path(filename=""):
    """Locates bundled assets in source or PyInstaller builds across OS platforms."""
...
def get_profile_data_path(folder_name):
    """
    Cross-platform persistent app data storage:
      - Windows: AppData/Local/NewMoon/NewMoonBrowser
      - macOS: ~/Library/Application Support/NewMoon/NewMoonBrowser
      - Linux: ~/.local/share/NewMoon/NewMoonBrowser
    """
    ...
    if not base_path:
        base_path = os.path.join(os.path.expanduser("~"), ".newmoon_browser")
...
class PaleMoonChromiumBrowser(QMainWindow):
    def __init__(self, is_far_side=False):
        super().__init__()
        self.is_far_side = is_far_side
        
        # Set Window Title & Icon
        title_suffix = " (Far Side)" if self.is_far_side else ""
        self.setWindowTitle(f"NewMoon Browser v1.1.2{title_suffix}")
        ...
        if self.is_far_side:
            ...
            self.base_color = "#181825"
            self.text_color = "#cdd6f4"
            self.accent_color = "#cba6f7"
            self.accent_hover = "rgba(203, 166, 247, 0.3)"
        else:
        ...
        # WebEngine Profiles Setup
        if self.is_far_side:
            # Incognito / Far Side uses an off-the-record in-memory profile
            self.profile = QWebEngineProfile("FarSideProfile", self)
        else:
            # Main Profile uses persistent disk storage
            storage_path = get_profile_data_path("DefaultProfile")
            cache_path = get_profile_data_path("Cache")
            self.profile = QWebEngineProfile("DefaultProfile", self)
            self.profile.setPersistentStoragePath(storage_path)
            self.profile.setCachePath(cache_path)

        # Download Manager setup and profile hook
        ...
        self.far_side_windows = []

    def setup_download_shortcut(self):
        """Global Ctrl+J shortcut to show the Downloads Manager."""
        shortcut = QShortcut(QKeySequence("Ctrl+J"), self)
        shortcut.activated.connect(lambda: self.download_manager.show())

    def on_download_requested(self, download_item: QWebEngineDownloadRequest):
        """Handles download requests and passes them to the Download Manager GUI."""
        default_dir = QStandardPaths.writableLocation(QStandardPaths.StandardLocation.DownloadLocation)
        suggested_path = os.path.join(default_dir, download_item.downloadFileName())

        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save File As", suggested_path
        )
    ...
    def open_far_side_window(self):
        far_side_win = PaleMoonChromiumBrowser(is_far_side=True)
        far_side_win.show()
        self.far_side_windows.append(far_side_win)
    ...
    def create_menu_bar(self):
        ...
        far_side_action = QAction("🌑 New Far Side Window", self)
        far_side_action.setShortcut(QKeySequence("Ctrl+Shift+N"))
        far_side_action.triggered.connect(self.open_far_side_window)
        file_menu.addAction(far_side_action)

        downloads_action = QAction("Downloads", self)
        downloads_action.setShortcut(QKeySequence("Ctrl+J"))
        downloads_action.triggered.connect(lambda: self.download_manager.show())
        ...
        add_bm_action.setShortcut(QKeySequence("Ctrl+D"))
    ...
    def on_title_changed(self, title, browser):
        for i in range(self.tabs.count()):
            ...
            if wrapper and wrapper.findChild(QWebEngineView) == browser:
                prefix = "🌑 " if self.is_far_side else ""
                short_title = title[:18] + "..." if len(title) > 18 else title
                self.tabs.setTabText(i, f"{prefix}{short_title}")
                break
    ...
    def create_toolbar(self):
        ...
        self.far_side_btn = QPushButton("🌑")
        self.far_side_btn.setFixedSize(28, 28)
        self.far_side_btn.setToolTip("Open New Far Side Window (Private Browsing)")
        self.far_side_btn.clicked.connect(self.open_far_side_window)
        ...
        nav_layout.addWidget(self.far_side_btn)
